...
GameView::GameView(EditorUI* editorUI, DX11Renderer* renderer) : EditorWindow("Game View", editorUI, renderer) {}
...
void GameView::Update(float deltaTime, RenderPipeline* pipeline)
{
	// 1. 씬에서 MainCamera를 찾음
	Camera* gameCam = GameEngine::Instance()->GetGameCamera();
	if (!gameCam || !pipeline || !m_renderer) return;
	...
	req.rtv = m_renderer->GetGameRTV(); // 게임용 RTV
...
void GameView::OnGUI()
{
	...
	Camera* gameCam = GameEngine::Instance()->GetGameCamera();
	if (gameCam->gameObject()->GetActive())
	{
		if (gameSRV)
		{
