...
CollisionInfo CollisionHelper::BoxVsBox(Collider* a, Collider* b)
{
	...
	if (xOverlap < yOverlap && xOverlap < zOverlap)
	{
		...
		// A가 B의 왼쪽 -> 법선은 오른쪽(+1)이어야 A에서 B를 향함
...
CollisionInfo CollisionHelper::CapsuleVsSphere(Collider* a, Collider* b)
{
	...
	Vector3 ab = B - A;
	float t = Vector3::Dot(C - A, ab) / ab.LengthSquared(); // 이건 공식상 어쩔 수 없이 제곱이 들어감(분모)
...
CollisionInfo CollisionHelper::CapsuleVsBox(Collider* a, Collider* b)
{
	...
	Vector3 bestCapsuleP;
	Vector3 bestBoxQ;

	const int samples = 8; // 샘플링 수를 조금 늘려 정밀도를 높입니다.
	for (int i = 0; i <= samples; ++i)
	{
		float t = (float)i / samples;
		Vector3 p = p1 + (p2 - p1) * t;

		// 박스 내에서 p와 가장 가까운 점 Q 찾기
		Vector3 q(
		...
		float dist = (q - p).Length();
		if (dist < minDistance)
		{
			minDistance = dist;
			bestCapsuleP = p; // 캡슐 위의 점
			bestBoxQ = q;     // 박스 위의 점
		}
	}
	...
	if (minDistance <= r)
	{
		...
		// 핵심: 법선 방향은 "캡슐 지점(P)에서 박스 지점(Q)을 향하는 방향"
		// 만약 두 점이 거의 겹쳤다면(완전 침범), 기존처럼 중심 기반 근사치 사용
		Vector3 diff = bestBoxQ - bestCapsuleP;
		float d = diff.Length();

		if (d > 0.0001f)
			info.normal = diff.Normalized();
		else
			info.normal = (box->GetCenter() - capsule->GetCenter()).Normalized();

		info.penetration = r - minDistance;
	}
	return info;
}
...
float CollisionHelper::ClosestPointsBetweenSegments(Vector3 p1, Vector3 p2, Vector3 q1, Vector3 q2, Vector3& outP1, Vector3& outP2)
{
	...
	return (outP1 - outP2).LengthSquared(); // 여기서는 공식 내부적으로 제곱을 반환 (나중에 루트 씌움)
}